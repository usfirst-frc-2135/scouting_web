...
function getAliasFromTeamNum(teamnum, teamAliasList) {
  ...
  for (let entry of teamAliasList) {
    ...
    console.log( "---> comparing teamnum " + teamnum + " with tnum " + tnum);
     if (tnum === teamnum) {
        rtnAlias = entry["aliasnumber"]
        console.log("   ---> FOUND teamnum, got alias = " + rtnAlias);
        break;
     }
    }
    return rtnAlias;
  ...
  function getTeamNumFromAlias(alias, teamAliasList) {
    ...
    for (let entry of teamAliasList) {
      ...
      console.log( "---> comparing alias " + alias + " with " + anum);
       if (anum == alias) {
         rtnTeamNum = entry["teamnumber"]
         console.log("   ---> FOUND alias, got teamnum = " + rtnTeamNum);
         break;
       }
      }
      return rtnTeamNum;
