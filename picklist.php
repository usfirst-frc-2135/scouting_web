  ...
  */
    ...
    handleScroll() {
      ...
      switch (this.state) {
        ...
        case 1: // Low start
          if (this.y > this.lowTresh) {
            this.state = 0;
          } else if (this.primaryCounter >= this.primaryCounterHit) {
            this.state = 3;
          }
          this.primaryCounter++;
          break;
        case 2: // Scroll wait
          if (this.y > this.lowTresh) {
            this.state = 0;
          } else if (this.secondaryCounter >= this.secondaryCounterHit) {
            this.state = 3;
          }
  ...
  function initialize_app_after_auth() {
    ...
    pickListSortable = new Sortable(document.getElementById('picklistDiv'), {
      ...
      handle: '.pickHandle',
      onStart: function() {
        scrollHandlerObj.startDrag()
      },
      onEnd: function() {
        scrollHandlerObj.endDrag();
        tableManualChange()
      },
      setData: function() {},
      onMove: function(evt, originalEvent) {
        scrollHandlerObj.setY(originalEvent.clientY)
      }
